...
public sealed class InMemoryGrantUseStoreIsolationTests
{
    ...
    {
        var store = new InMemoryCapabilityGrantUseStore { EvictionGracePeriod = TimeSpan.FromMinutes(1) };
        CapabilityTokenGrant grant = CreateGrant("issuer-1", "expiring", expiresUtc: Now);
    ...
    {
        var store = new InMemoryCapabilityGrantUseStore { EvictionGracePeriod = TimeSpan.FromMinutes(1) };
        CapabilityTokenGrant grant = CreateGrant("issuer-1", "skewed", expiresUtc: Now);
        SignedGovernanceArtifact<CapabilityTokenGrant> signedGrant = CreateUnsignedArtifact(grant);
        ...
        Assert.Equal(CapabilityTokenValidationCategory.ReuseLimitExceeded, replay.Category);
    ...
    private static SignedGovernanceArtifact<CapabilityTokenGrant> CreateUnsignedArtifact(CapabilityTokenGrant grant)
    {
