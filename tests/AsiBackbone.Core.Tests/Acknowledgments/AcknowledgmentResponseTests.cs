...
/// <summary>
/// Unit tests for the <see cref="AcknowledgmentResponse"/> class, which represents the acknowledgment response to a acknowledgment request.
/// </summary>
public sealed class AcknowledgmentResponseTests
{
    /// <summary>
    /// Verifies that the <see cref="AcknowledgmentResponse.Accept"/> method creates an acknowledgment with the expected properties when accepting a handshake request.
    /// </summary>
    [Fact]
    public void AcceptCreatesAcknowledgedResponse()
    {
        var actor = GovernanceActorContext.Human("user-123", "Chris");
        AcknowledgmentRequest request = CreateRequest(actor);

        var acknowledgment = AcknowledgmentResponse.Accept(
            request,
            actor,
    ...
    /// <summary>
    /// Verifies that the <see cref="AcknowledgmentResponse.Reject"/> method creates an acknowledgment with the expected properties when rejecting a handshake request.
    /// </summary>
    [Fact]
    public void RejectCreatesRejectedResponse()
    {
        ...
        var acknowledgment = AcknowledgmentResponse.Reject(
            request,
            actor,
            acknowledgmentId: "acknowledgment-123");
    ...
    /// <summary>
    /// Verifies that the <see cref="AcknowledgmentResponse.Accept"/> method generates a non-empty acknowledgment ID when one is not provided.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// Verifies that the <see cref="AcknowledgmentResponse.Accept"/> method generates a non-empty acknowledgment ID when a whitespace identifier is provided.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// Verifies that the <see cref="AcknowledgmentResponse.Accept"/> method trims a provided acknowledgment identifier.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// Verifies that the <see cref="AcknowledgmentResponse.Accept"/> method normalizes the provided timestamp to UTC.
    /// </summary>
    [Fact]
    ...
    public void CreateNormalizesMissingOptionalValuesToNull()
    {
        ...
        AcknowledgmentRequest request = CreateRequestWithoutTrace(actor);
    ...
    /// <summary>
    /// Verifies that the <see cref="AcknowledgmentResponse.Accept"/> method normalizes metadata keys and values by trimming whitespace and ignoring empty keys.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// Verifies that the <see cref="AcknowledgmentResponse.Accept"/> method throws an <see cref="ArgumentNullException"/> when the request parameter is null.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// Verifies that the <see cref="AcknowledgmentResponse.Accept"/> method throws an <see cref="ArgumentNullException"/> when the actor parameter is null.
    /// </summary>
    [Fact]
    public void CreateThrowsForMissingActor()
    {
        AcknowledgmentRequest request = CreateRequest(GovernanceActorContext.System);
    ...
    private static AcknowledgmentRequest CreateRequest(IGovernanceActorContext actor)
    {
        return AcknowledgmentRequest.Create(
            actor,
            "document.approve",
            "ack.required",
    ...
    private static AcknowledgmentRequest CreateRequestWithoutTrace(IGovernanceActorContext actor)
    {
        return AcknowledgmentRequest.Create(
            actor,
            "document.approve",
