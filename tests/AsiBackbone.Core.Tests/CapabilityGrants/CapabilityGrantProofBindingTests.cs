...
public sealed class CapabilityGrantProofBindingTests
{
    ...
    public async Task ValidateAsyncDeniesGrantWhoseScopesWereWidenedAfterSigning()
    {
        ...
        CapabilityGrant widenedGrant = CreateGrant(scopes: ["robotics.execute", "robotics.admin"]);

        SignedGovernanceArtifact<CapabilityGrant> tampered = SignedGovernanceArtifacts.FromSigningMetadata(
            widenedGrant,
            CanonicalPayloadBuilder.ForCapabilityGrant(widenedGrant),
        ...
        Assert.Equal(CapabilityGrantValidationCategory.InvalidProof, result.Category);
    ...
    {
        SignedGovernanceArtifact<CapabilityGrant> signed = CreateSignedGrant(CreateGrant());
        CapabilityGrant extendedGrant = CreateGrant(expiresUtc: IssuedUtc.AddDays(30));

        SignedGovernanceArtifact<CapabilityGrant> tampered = SignedGovernanceArtifacts.FromSigningMetadata(
            extendedGrant,
            CanonicalPayloadBuilder.ForCapabilityGrant(extendedGrant),
            signed.CanonicalHash,
            signed.SigningMetadata);
    ...
    public async Task ValidateAsyncDeniesProofIssuedForADifferentArtifactType()
    {
        ...
        SignedGovernanceArtifact<CapabilityGrant> artifact = SignedGovernanceArtifacts.FromSigningMetadata(
            grant,
            foreignPayload,
            foreignHash,
    ...
    {
        CapabilityGrant otherGrant = CreateGrant(tokenId: "grant-other");
        CapabilityGrant grant = CreateGrant();

        CanonicalPayload otherPayload = CanonicalPayloadBuilder.ForCapabilityGrant(otherGrant);
