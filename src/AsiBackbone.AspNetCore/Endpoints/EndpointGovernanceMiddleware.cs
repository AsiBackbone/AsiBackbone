...
/// </summary>
public sealed class EndpointGovernanceMiddleware
{
    ...
    private readonly EndpointGovernanceOptions options;

    /// <summary>
    /// Initializes a new instance of the <see cref="EndpointGovernanceMiddleware" /> class.
    /// </summary>
    /// <param name="next">The next request delegate.</param>
    /// <param name="endpointOptions">The endpoint governance options.</param>
    public EndpointGovernanceMiddleware(
    ...
    public async Task InvokeAsync(
        ...
        IEndpointGovernanceService governanceService)
    {
    ...
    private static IResult CreateDefaultForbiddenResult(
        ...
        EndpointGovernanceResult? result,
        string decisionStage)
    {
        return AsiBackboneEndpointGovernanceDevelopmentDiagnostics.IsEnabled(httpContext, options)
            ...
            : options.DefaultForbiddenResultFactory(httpContext)
                ?? throw new InvalidOperationException($"{nameof(EndpointGovernanceOptions.DefaultForbiddenResultFactory)} must return a non-null result.");
    }
}
