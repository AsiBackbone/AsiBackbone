...
public sealed class HttpGovernanceActorContextOptions
{
    ...
    /// </summary>
    /// <remarks>
    /// The conservative default accepts only <see cref="GovernanceActorType.Human" />. A host must explicitly add <see cref="GovernanceActorType.System" />, <see cref="GovernanceActorType.Service" />, or <see cref="GovernanceActorType.Agent" /> after establishing that the configured claim is protected by a trusted identity boundary. An empty list disables actor-type claim mapping.
    /// </remarks>
    public IList<GovernanceActorType> AllowedActorTypesFromClaims { get; set; } = [.. DefaultAllowedActorTypesFromClaims];

    /// <summary>
    /// Gets or sets the actor type used when an authenticated principal does not provide a valid or allowed actor type claim.
    /// </summary>
    public GovernanceActorType DefaultAuthenticatedActorType { get; set; } = GovernanceActorType.Human;
