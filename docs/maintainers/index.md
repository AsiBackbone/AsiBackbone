# Maintainer & Quality Evidence

This area contains release-readiness, quality, benchmark, validation, and maintenance evidence for AsiBackbone.

These records are public for traceability, but they are not part of the normal consumer implementation path. Current package/API behavior remains authoritative in the product documentation.

## Current release evidence

- [3.2.2 Release Readiness Record](../articles/release-readiness-322.md)
- [Release Cadence and Readiness](../articles/release-cadence-and-readiness.md)
- [Stable Release Validation](../articles/release-validation.md)
- [Quality Reports](../quality/index.md)

## Quality and engineering evidence

- [Performance Benchmark Baseline](../articles/performance-benchmark-baseline.md)
- [Policy Evaluator Allocation Review](../articles/policy-evaluator-allocation-review.md)
- [EF Core Outbox Bulk-Claim Performance Evaluation](../articles/efcore-outbox-bulk-claim-evaluation.md)
- [API Baseline and Boundary Checks](../articles/api-baseline-and-boundary-checks.md)
- [Public API XML Documentation](../articles/public-api-xml-documentation.md)
- [Production Placeholder Exception Guardrails](../articles/production-placeholder-exception-guardrails.md)
- [Post-2.0.0 Documentation Currency Review](../articles/post-200-documentation-currency-review.md)
- [Implicit Usings Posture](../articles/implicit-usings-posture.md)
- [Developer Checklist](../articles/developer-checklist.md)
- [Cryptographic Security Hardening Roadmap](../articles/cryptographic-security-hardening-roadmap.md)

## Historical release-readiness archive

The records below preserve the release gate and audit trail for prior stable versions. They are not current release status.

- [3.2.1 Release Readiness Record](../articles/release-readiness-321.md)
- [3.2.0 Release Readiness Record](../articles/release-readiness-320.md)
- [3.1.0 Release Readiness Record](../articles/release-readiness-310.md)
- [3.0.1 Release Readiness Record](../articles/release-readiness-301.md)
- [3.0.0 Release Readiness Record](../articles/release-readiness-300.md)
- [2.3.0 Release Readiness Record](../articles/release-readiness-230.md)
- [2.2.1 Release Readiness Record](../articles/release-readiness-221.md)
- [2.2.0 Release Readiness Record](../articles/release-readiness-220.md)
- [2.1.0 Release Readiness Record](../articles/release-readiness-210.md)
- [2.0.2 Release Readiness Record](../articles/release-readiness-202.md)
- [2.0.1 Release Readiness Record](../articles/release-readiness-201.md)
- [2.0.0 Release Readiness Record](../articles/release-readiness-200.md)
- [1.2.1 Release Readiness Record](../articles/release-readiness-121.md)
- [1.2.0 Release Readiness Record](../articles/release-readiness-120.md)

For alpha-era and superseded design/readiness records, use [Historical and Superseded Records](../history/).
