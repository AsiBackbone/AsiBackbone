...
| `AsiBackbone.AspNetCore` | `AsiBackbone.AspNetCore.Endpoints.RequireGovernancePolicyAttribute` | Retain: existing domain/host role is clear. |
...
These managed API changes rename types and update receipt diagnostic wording. The default validation label on GovernanceDecisionContract.VerifyAuditResidue is now "Decision receipt". Package IDs, namespaces, registration method names, protocol member names, JSON keys, schema versions, canonical artifact tags, signing payloads, diagnostic IDs/host opt-in markers, and EF table/column names retain their established contracts. In particular, AuditResidueId and ForAuditResidue/SignAuditResidueAsync remain protocol/helper member names; renaming a receipt type does not rewrite stored evidence or signed bytes. Historical release/migration records retain 5.x type names.
