...
| --- | --- | --- | --- |
| Policy decision pipeline | Composition of constraints, evaluator, decision, audit, acknowledgment/capability surfaces, and host execution boundaries | `AsiBackbone.Core`; [Policy Evaluator Pipeline](policy-evaluator-pipeline.md) | “Governance spine” remains architectural positioning, not a required implementation term or public type. |
...
| Outbox | Durable pending records for reliable governance-event delivery | `AsiBackbone.EntityFrameworkCore` integration and outbox guides | `GovernanceOutbox*` API names identify the specialized records; production persistence and concurrency semantics are product-owned. |
